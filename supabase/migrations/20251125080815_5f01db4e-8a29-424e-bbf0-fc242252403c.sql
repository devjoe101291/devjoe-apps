...
-- Update apps table policies to require admin role
DROP POLICY IF EXISTS "Authenticated users can create apps" ON public.apps;
...
CREATE POLICY "Only admins can create apps"
ON public.apps
FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));
